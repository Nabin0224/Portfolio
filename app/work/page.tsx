...
import {project1Img} from "@/components/ui/data/workImg"
  ...
  return (
      ...
      <div className="skills flex flex-col flex-1 mx-2 md:mx-6 my-4 md:my-0 p-1 md:p-0 ">
        {skills &&
          skills.map((skill, index) => (
            <HoverTooltip text="Expand" key={index} >
            <div className=""  key={index}>
              ...
              >
                ...
                <Link href={skill.website} target="_blank">
                  <div className="no-tooltip" onClick={(e)=> e.stopPropagation()} >
              ...
              <AnimatePresence>
                {
                  ...
                  >
                 <div className={`flex gap-1 relative w-full overflow-hidden py-2 bg-black `}>
                  {
                    project1Img.map((item, idx) => (
                    //  <div className="imgWrapper h-80 w-96"
                    //   style={{ backgroundImage : `url(${ecomImg.src})` }} >
                    //    <img key={idx} src={item.src} alt="project1 image" className="h-60 w-[400px] object-contain" />
                    //  </div>

                    <motion.div className="min-w-[400px] h-80 flex justify-center items-center px-4"
                    style={{ backgroundImage : `url(${ecomImg.src})` }}
                    animate={{ x: ["0%", "-100%"] }}
                    transition={{
                      duration: 10,
                  ...
                  >
                   <img key={idx} src={item.src} alt="project1 image" className="h-60 w-[800px] object-contain" />
                  </motion.div>
                    ))
                  }
                 </div>
    ...
    <section className="">
      ...
      <div className="text-center font-sans text-4xl md:text-[52px] text-neon mt-10 leading-15">
        <p className="text-center">To talk collaborations or partnerships, send an email to 
        <span className="relative inline-block group cursor-pointer"
        onMouseEnter={playAudio}
        >
                      <span className="relative inline-block group text-neon">
                        
          <a href={`mailto:nabinbudhathoki2004@gmail.com`} className="hover:text-primary">nabinbudhathoki2004@gmail.com</a>
                      </span>
